...
import {
  selectDataUserReducer,
  UserActions
} from "pages/Dashboard/User/module/UserSlice";
...
export default function UserTable(props: UserProps) {
  const dispatch = useAppDispatch();
  const { classes } = useStyles();
  const [page, setPage] = React.useState(0);
  const [rowsPerPage, setRowsPerPage] = React.useState(10);
  const [search, changeSearch] = useState<string>("");
  const data = useAppSelector(selectDataUserReducer);
