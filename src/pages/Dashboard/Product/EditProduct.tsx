...
const EditProduct = (props: Props) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const response = await productApi.getById(id, token);
        setData(response);
        setSelectValuesMultipleSelectSizes(
          response.sizes.map((item: any, index: number) => {
            return item.size;
          })
        );
        setSelectValuesMultipleSelectColor(
          response.imgDetails.map((item: any, index: number) => {
            return item.color;
          })
        );
      } catch (error) {
  ...
  const onSubmitForm = async (e: React.SyntheticEvent) => {
    ...
    try {
      const token = getToken();
      const response = await productApi.update(data, token);
